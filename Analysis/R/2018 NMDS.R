...
# import data
bugs.by.year <- read_csv("../Data/bugs.by.year.csv") %>% 
  mutate_at(vars(CollDate), funs(factor))
bugs.agg.zero <- read_csv("../Data/bugs.agg.zero.csv") %>% 
  mutate_at(vars(CollDate), funs(factor))
bugs18 <- bugs.agg.zero %>% filter(CollDate == "2018") %>% 
  mutate_at(vars(CollDate), funs(factor))

# Spread data to get a taxon matrix, create factor of CollDate and Treatment
bugs.mds <- bugs.agg.zero %>% 
  select(-c("FFG", "Size")) %>%
