...
ffg.ratios %>%  group_by(Stream, Year) %>% summarise_at(vars(Difference, Ratio, `Log(Ratio) Year`), mean) %>% 
  filter(Stream == "W-113") %>% 
  ggplot(aes(x = Year, y = `Log(Ratio) Year`, fill = as.factor(Year))) +
  geom_col() +
...
ffg.ratios %>%  group_by(Stream, Year) %>% summarise_at(vars(Difference, Ratio, `Log(Ratio) Year`), mean) %>% 
  ggplot(aes(x = Stream, y = `Log(Ratio) Year`, fill = as.factor(Year))) +
  geom_col(position = "dodge") +
...
avg.ratio <- ffg.ratios %>% group_by(Stream, Year) %>% summarise_at(vars(Difference, Ratio, `Log(Ratio) Year`), mean)
avg.ratio <- data_summary(data = avg.ratio, varname = "Log(Ratio) Year", 
                                groupnames = c("Year"))
avg.ratio %>% 
  ggplot(aes(x = Year, y = Mean, fill = as.factor(Year))) +
  geom_col(position = "dodge") +
...
ffg.avg.ratio <- data_summary(data = ffg.ratios, varname = "Log(Ratio) Year", 
                                groupnames = c("Year", "FFG"))
ffg.avg.ratio %>% 
  ggplot(aes(x = FFG, y = Mean, fill = as.factor(Year))) +
  geom_col(position = "dodge") +
  geom_errorbar(aes(x = FFG, ymin = Mean - SD, ymax = Mean + SD), position = "dodge") +
  labs(title = "Average of the log ratio \n (Treatment / Control)",
       x = "Treatment",
       y = "Log ratio") +
  scale_fill_grey()
```
