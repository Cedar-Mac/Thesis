...
bugs.by.year <- bugs.agg %>% select(-c("Density", "CollDate")) %>% unique()

# Calculate Differences
bugs.by.year$Diff <- bugs.agg$Density[bugs.agg$CollDate == "2018"] -     
...
ggplot(data = bugs.by.year, aes(x = Taxon, y = Diff, fill = Treatment)) + 
...
write_csv(bugs.mds, "~/bugsmds.csv")
```
...
group_by(bugs.by.year, Stream, Treatment) %>%
  summarise_at(vars(Diff), funs(sum)) %>% 
  arrange(Treatment) %>% 
