...
```{r Bentho torch 2018, fig.height = 10, fig.width = 12, echo = TRUE}
...
bentho.mean <- aggregate(BenthoTotal ~ Stream + Treatment Year.treat+ Meter, mean, data = Bentho)
...
```{r Bentho torch Both Years, fig.height = 10, fig.width = 12, echo = TRUE}
# Change variables to factors.  Re-order with CHUCK at the end
...
write_csv(bentho.reach, "./Data/bentho.reach")
...
### Taxa
***
```{r Plot Taxon Density Diffs, echo = TRUE, fig.height = 25, fig.width = 25}
...
```{r Plot FFG Density Differences, fig.height = 10, fig.width = 12, echo = TRUE}
...
```{r Plot 2018 FFG Densities, fig.height = 10, fig.width = 12, echo = TRUE}
bugs18 %>%
...
```{r Plot 2018 Scraper Taxa, fig.height = 10, fig.width = 12, echo = TRUE}
...
```{r Plot Size Differences, fig.height = 10, fig.width = 12, echo = TRUE}
bugs.reach.diff %>% 
...
```{r Make Grouping and Taxon Matrix, echo = TRUE}
...
```{r Plot Benthic NMS, fig.height = 10, fig.width = 12, echo = TRUE}
...
```{r Total Densities, fig.height=10, fig.width=12, echo=FALSE}
...
```{r Density Diffs, fig.height=10, fig.width=12, echo=FALSE}
...
```{r FFG Densities, fig.height=10, fig.width=12, echo=FALSE}
...
```{r Terrestrial Vs. Aquatic, echo =TRUE}
Diets.reach %>%
  group_by(Stream, Treatment, Origin) %>%
  summarise_at(vars(Count), funs(sum)) %>% 

  
ggplot(aes(x = Origin, y = Count, fill = Treatment)) +
  geom_col() +
  labs(title = "Total Aquatics and Terrestricals by Reach") +
    theme(legend.position = c(.8, .8), legend.key.width = unit(5, "cm")) +
    scale_color_viridis_d(option = "E")
...
```{r Plot Taxa Proportions, echo = TRUE, fig.height = 12, fig.width = 12}
...
```{r Plot FFG Proportions, echo = TRUE, fig.height = 12, fig.width = 12}
...
```{r Plot Size Proportions, echo = TRUE, fig.height = 12, fig.width = 12}
...
```{r Plot Costello, echo = TRUE, fig.height = 12, fig.width = 12}
...
```{r Make Diet Grouping and Taxon Matrix, echo = TRUE}
# Get grouping variables and taxon matrix
Enviro <- bugs.mds %>%
...
```{r Diet Mean NMS, echo = TRUE}
#Get mean x,y values 
NMDS.mean <- select(NMDS, x, y) %>% aggregate(grouping, mean)
```


```{r Diet ELlipse, echo = TRUE}
#Load ellipse for NMDS
plot.new()
...
```{r NMDS Plot6, fig.height = 10, fig.width = 12, echo = TRUE}
