...
ffg <- read_csv("2014-16 FFG.csv") 
ffg <- ffg[complete.cases(ffg),]
ffg <- ffg[19:27,]
ffg <- ffg[- c(3,6,9),]
ffg_enviro <- (read_csv("ffg_enviro.csv")) %>% mutate_if(is.character, as.factor)
ffg_enviro <- ffg_enviro[- c(3,6,9),]
# ffg_enviro[19:27, 3] = c("OG", "OG", "SG", "SG", "SG", "OG", "SG", "SG", "OG")

attach(ffg_enviro)

grouping <- select(ffg_enviro, Stream, Reach, Watershed)
meta_data <- ffg_enviro[,5:18]
meta_data[3] <- NULL

#Get MDS for invert density
sol <- metaMDS(ffg, distance = "bray", k = 2, trymax = 40, autotransform = TRUE)


#set up NMDS with dimensions of sol and env factors from "Enviro". 
NMDS <- data.frame(x = sol$point[, 1], y = sol$point[ ,2], Stream = select(grouping, Stream), 
...
p <- p + annotate("text", x = (NMDS.mean$x -.03), y = (NMDS.mean$y + .025), label = NMDS.mean$Stream, size = 2)
...
res <- bio.env(wisconsin(ffg), meta_data, fix.dist.method = fmethod, var.dist.method = vmethod, 
...
res.bv.step.bio <- bv.step(wisconsin(ffg), wisconsin(ffg), 
...
##
MDS_res = metaMDS(ffg, distance = nmethod, k = 2, trymax = 50)

bio.keep <- as.numeric(unlist(strsplit(res.bv.step.bio$order.by.best$var.incl[5], ",")))
bio.fit <- envfit(MDS_res, ffg[,bio.keep,drop = F], perm = 999)
...
for (i in 1:dim(meta_data)[2]){
  tmp <- data.frame(row.names = NULL, Sample = as.character(grouping$Stream), Richness = S.rar, Env = meta_data[i,])
  if (is.null(df_S)){
    df_S = tmp
  }else{
    df_S <- rbind(df_S, tmp)
...
library(plyr)
cors<-ddply(df_S,.(Env.Wetted.Average..m.), summarise, cor=round(cor(Richness, Env.),2), sig=formatPvalues(cor.test(Richness, Sample)$p.value))
