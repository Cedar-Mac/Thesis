...
x <- c("tidyverse", "vegan", "lubridate", "readxl", "ggthemes", "viridis")
...
plot.new()
ord <- ordiellipse(sol, NMDS$Time.Reach.Meter, display = "sites", kind = "sd", conf = 0.95, label = TRUE)
dev.off()

df_ell <- data.frame()
for(g in NMDS$Time.Reach.Meter){
  df_ell <- rbind(df_ell, cbind(as.data.frame(with(NMDS[NMDS$Time.Reach.Meter == g,],
